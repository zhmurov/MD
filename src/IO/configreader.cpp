...
void parseParametersFile(const char* filename, int argc, char *argv[]){
	//safe_fopen - читаем файл, путь к которому задан через filename
	//filename – указатель на строку содержащую имя (включая путь) открываемого файла.
	//mode ("r") – указатель на строку содержащую режим доступа к открытому файлу. r или rb – открытие файла для чтения.
	...
	//http://cppstudio.com/post/747/
			...
			char* pch = strtok(buffer, " \t");	//во втором параметре указаны пробел и табуляция

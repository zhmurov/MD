...
Harmonic::Harmonic(MDData *mdd, float ks, int count, int2* bonds, float* bondsR0){
	this->mdd = mdd;
	this->ks = ks;
...
//maxBonds
	...
	printf("Max bonds per atom (Harmonic potential) = %2d\n", maxBonds);
...
//================================================================================================
__global__ void harmonic_kernel(float ks, int* d_bondCount, int* d_bondMap, float* d_bondMapR0){
...
//================================================================================================
__global__ void harmonicEnergy_kernel(float ks, int* d_bondCount, int* d_bondMap, float* d_bondMapR0, float* d_energy){
	int i = threadIdx.x + blockIdx.x*blockDim.x;
	if (i < c_mdd.N){
...
//================================================================================================
void Harmonic::compute(){
	harmonic_kernel<<<this->blockCount, this->blockSize>>>(ks, d_bondCount, d_bondMap, d_bondMapR0);
}

//================================================================================================
float Harmonic::getEnergies(int energyId, int timestep){
	harmonicEnergy_kernel<<<this->blockCount, this->blockSize>>>(ks, d_bondCount, d_bondMap, d_bondMapR0, d_energy);

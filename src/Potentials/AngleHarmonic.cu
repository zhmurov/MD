...
#include "AngleHarmonic.cuh"

/*
 * Constructor for harmonic angle potential
 * \param mdd is the pointer to the main MDData object
 * \param angleCount is a total number of harmonic angles in the system
 * \param angles is an int3 array of size angleCount, containing i-j-k indexes of angled atoms as x,y and z of the int3 type
 * \param angleParameters is a float2 array of size angleCount, containing harmonic angle parameters: the equilibrium angle theta0 as x and Ktheta as y fields of float2
 *
 */
AngleHarmonic::AngleHarmonic(MDData *mdd, int angleCount, int3* angles, float2* angleParameters){
	printf("Initializing harmonic Angle potential\n");
	this->mdd = mdd;
	...
	for(a = 0; a < angleCount; a++){
		...
		h_angles[a].y = j;
		h_angles[a].z = k;
		h_pars[a].x = angleParameters[a].x;
		h_pars[a].y = angleParameters[a].y;
		h_refs[a].x = h_count[i];
		h_refs[a].y = h_count[j];
		h_refs[a].z = h_count[k];
		h_count[i]++;
		h_count[j]++;
		h_count[k]++;
	}
	...
	int maxAngles = 0;
	for(a = 0; a <= lastAngled; a++){
		if(h_count[a] > maxAngles){
			maxAngles = h_count[a];
		}
	}
...
__global__ void angleHarmonicCompute_kernel(int4* d_angles, int4* d_refs, float2* d_pars, float4* d_forces, int widthTot, int angleCount){
...
__global__ void angleHarmonicSum_kernel(int* d_count, float4* d_forces, int widthTot, int lastAngled){
	int d_i = blockIdx.x*blockDim.x + threadIdx.x;
	if(d_i <= lastAngled){
...
void AngleHarmonic::compute(){
	angleHarmonicCompute_kernel<<<this->blockCount, this->blockSize>>>(d_angles, d_refs, d_pars, d_forces, widthTot, angleCount);
	//cudaThreadSynchronize();
	angleHarmonicSum_kernel<<<blockCountSum, blockSizeSum>>>(d_count, d_forces, widthTot, lastAngled);
}


__global__ void angleHarmonicEnergy_kernel(int4* d_angles, float2* d_pars, float* d_energies, int angleCount){
	int d_i = blockIdx.x*blockDim.x + threadIdx.x;
	if(d_i < angleCount){
		...
		float theta = acos(costheta);
		float diff = theta - par.x;
		float pot = par.y*diff*diff;
...
float AngleHarmonic::getEnergies(int energyId, int timestep){
	angleHarmonicEnergy_kernel<<<this->blockCount, this->blockSize>>>(d_angles, d_pars, d_energies, angleCount);
	//cudaThreadSynchronize();

...
LeapFrog_new::LeapFrog_new(MDData *mdd, float T, float seed, int* h_fixAtoms){
...
__global__ void integrateLeapFrogUnderdumped_kernel(float* d_gamma, float* d_var, int* d_fixAtoms){
	...
	if(d_i < c_mdd.N){
		...
		vel.x += df*(f.x - vel.x*gamma + var*rf.x);
		vel.y += df*(f.y - vel.y*gamma + var*rf.y);
		vel.z += df*(f.z - vel.z*gamma + var*rf.z);
...
void LeapFrog_new::integrateStepTwo (){
	integrateLeapFrogUnderdumped_kernel<<<this->blockCount, this->blockSize>>>(d_gamma, d_var, d_fixAtoms);
}

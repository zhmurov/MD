...
PushingSphere::PushingSphere(MDData *mdd, float R0, float R, float4 centerPoint, int updatefreq, float sigma, float epsilon, const char* outdatfilename, int lj_or_harmonic){
...
__global__ void PushingSphereLJ_Compute_kernel(float* d_p_sphere, float R, float4 r0, float sigma, float epsilon){
...
__global__ void PushingSphereHarmonic_Compute_kernel(float* d_p_sphere, float R, float4 r0, float sigma, float epsilon){
	int d_i = blockIdx.x*blockDim.x + threadIdx.x;
	if (d_i < c_mdd.N){
		if(c_mdd.d_atomTypes[d_i] == 0){	//FOR DNA
			float k = epsilon;
...
void PushingSphere::compute(){
	...
	if(lj_or_harmonic == PUSHING_SPHERE_LJ){
		PushingSphereLJ_Compute_kernel<<<this->blockCount, this->blockSize>>>(d_p_sphere, this->radius, this->centerPoint, this->sigma, this->epsilon);
	}
	if(lj_or_harmonic == PUSHING_SPHERE_HARMONIC){
		PushingSphereHarmonic_Compute_kernel<<<this->blockCount, this->blockSize>>>(d_p_sphere, this->radius, this->centerPoint, this->sigma, this->epsilon);
	}
...
__global__ void PushingSphereLJ_Energy_kernel(float* d_energy, float R, float4 r0, float sigma, float epsilon){
...
__global__ void PushingSphereHarmonic_Energy_kernel(float* d_energy, float R, float4 r0, float sigma, float epsilon){
	int i = threadIdx.x + blockIdx.x*blockDim.x;
	if (i < c_mdd.N){
		float k = epsilon;
		float4 ri = c_mdd.d_coord[i];
...
float PushingSphere::get_energies(int energy_id, int timestep){
	...
	if(lj_or_harmonic == PUSHING_SPHERE_LJ){
		PushingSphereLJ_Energy_kernel<<<this->blockCount, this->blockSize>>>(d_energy, this->radius, this->centerPoint, this->sigma, this->epsilon);
	}
	if(lj_or_harmonic == PUSHING_SPHERE_HARMONIC){
		PushingSphereHarmonic_Energy_kernel<<<this->blockCount, this->blockSize>>>(d_energy, this->radius, this->centerPoint, this->sigma, this->epsilon);
	}
